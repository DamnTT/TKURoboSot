...
{
  //USB
  g_usNodeId = 1; //Origin: 1
  g_deviceName = "EPOS2"; //Origin: EPOS4
  g_protocolStackName = "MAXON SERIAL V2"; 
  g_interfaceName = "USB"; 
  g_portName = "USB0"; //Origin: USB0
  g_baudrate = 1000000; 
}
...
bool MyProfileVelocityMode(HANDLE p_DeviceHandle, unsigned short p_usNodeId, unsigned int & p_rlErrorCode)
{
  int lResult = MMC_SUCCESS;
  stringstream msg;
...
int MotorSetting(unsigned int* p_pErrorCode)
{
  ...
  if ((lResult = VCS_SetMotorType(g_pKeyHandle, g_usNodeId, MT_DC_MOTOR, p_pErrorCode)) != MMC_SUCCESS) {
    LogError("VCS_SetMotorType", lResult, *p_pErrorCode);
    return lResult;
  }else {
    LogInfo("VCS_SetMotorType OK,\nSetting DC Motor Parameter and Sensor Type...");
    ...
    // const unsigned short nominal_current = 4; // mA
    // const unsigned short maxoutput_current = 5; // mA
    // const unsigned short thermal_time_constant = 41.5;
    // if ((lResult = VCS_SetDcMotorParameter(g_pKeyHandle, g_usNodeId,
    //               nominal_current, maxoutput_current,
    //               thermal_time_constant, p_pErrorCode)) != MMC_SUCCESS) {
    //   LogError("VCS_SetDcMotorParameter", lResult, *p_pErrorCode);
    //   return lResult;
    // }
  }
  return lResult;
...
int MyDemo(unsigned int* p_pErrorCode)
{
  int lResult = MMC_SUCCESS;
  unsigned int lErrorCode = 0;

  lResult = MyProfileVelocityMode(g_pKeyHandle, g_usNodeId, lErrorCode);
  ...
  switch(g_eAppMode)
  {
    case AM_DEMO:
    {
      ...
      if ((lResult = MotorSetting(&ulErrorCode))!=MMC_SUCCESS) {
        LogError("MotorSetting", lResult, ulErrorCode);
        return lResult;
      }

      if((lResult = MyDemo(&ulErrorCode))!=MMC_SUCCESS)
      {
        LogError("MyDemo", lResult, ulErrorCode);
        return lResult;
      }
