  ...
  : it_(nh)
{
  ...
  frame2 = new cv::Mat(cv::Size(FRAME_COLS, FRAME_ROWS), CV_8UC3 );
...
//////////////////////////////////////////////////////////
void InterfaceProc::White_Line(const cv::Mat iframe)
{
  ...
  //Parameter_getting(1);
  for (double angle = FrontMsg; angle < FrontMsg + 360; angle = angle + WhiteAngleMsg) {
...
void InterfaceProc::Black_Line(const cv::Mat iframe)
{
  ...
  // Parameter_getting(1);
  for (int angle = FrontMsg; angle < FrontMsg + 360; angle = angle + BlackAngleMsg) {
    int angle_be = angle;
    //cout<<angle_be<<endl;
    if (angle_be > 360) angle_be -= 360;
